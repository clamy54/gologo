...
var helpData = map[string]helpEntry{
	...
	"SI":           {en: "IF", frAliases: []string{"SISINON"}, enAliases: []string{"IFELSE"}, params: "pred liste [liste2]", kind: "C", exemples: []string{"SI 2 > 1 [ ECRIS \"OUI ]", "SI :N = 0 [ ECRIS \"ZERO ] [ ECRIS \"AUTRE ]"}, descFr: "Conditionnelle : exécute liste si pred est VRAI. Avec une 2e liste : sinon exécute liste2. Alias SISINON (IFELSE).", descEn: "Conditional: runs list if pred is TRUE. With a 2nd list: otherwise runs list2. Alias IFELSE."},
